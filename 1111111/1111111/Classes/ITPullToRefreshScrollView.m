...
- (void)initialSetup {
    ...
    // 需要设置 self.contentView 可以发送Bounds 改变通知
    _running = NO;
...
//    if ([_delegate respondsToSelector:@selector(scrollViewDidContentOffsetChange:contentOffSet:)]) {
//        [_delegate scrollViewDidContentOffsetChange:self contentOffSet:self.contentView.documentVisibleRect.origin];
//    }
//    
//    if (NSEqualPoints(self.contentView.bounds.origin, self.contentView.documentVisibleRect.origin) == NO) {
...
#pragma mark ---  滚动时候判断头部还是底部
    ...
    else
    {
        ...
        if (  positionAxisY < 10 && positionAxisY > 8) {
            NSLog(@"-=1-= %f", positionAxisY);
            [self aaa];
        } else if (positionAxisY < -8) {
            NSLog(@"-=2-= %f", positionAxisY);
            [self aaa];
        }
        
        
//        lastStamp = mach_absolute_time(); // get current timestamp
//        if (!_running)
//        {
//            if (!timer)
...
- (void)createTimerSource {
    dispatch_queue_t _timerQueue = dispatch_get_global_queue(0, 0);
    timer=dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, _timerQueue);
    dispatch_source_set_timer(timer, DISPATCH_TIME_NOW, 2 * NSEC_PER_SEC, 0);
    dispatch_source_set_event_handler(timer, ^{
        uint64_t stamp = mach_absolute_time(); // current stamp
//        NSLog(@"-=-00== %llu", (stamp - lastStamp));
        if (stamp - lastStamp > 130000)
        {
...
#pragma mark -- 走代理方法
        if ([_delegate respondsToSelector:@selector(scrollViewDidContentOffsetChange:contentOffSet:)]) {
            [_delegate scrollViewDidContentOffsetChange:self contentOffSet:CGPointMake(0, positionAxisY)];
        }
